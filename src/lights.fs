...
void main() {
    ...
    if (active.x == 1) {
        ...
        if (strength > 0) {
            finalColor = vec4(color.xyz - strength / 100, 1);
        } else {
            // finalColor = vec4(color.xyz + (color.a * 100) / (distance * 10), 1);
    ...
    // strength = (distance - strength * 10) / 500;
    // finalColor = vec4(color.xyz + strength, 1);
