...
float Shadow(float circle, float alpha, float distance) {
    if (circle > alpha * 100) {
        return 0;
    } else {
...
float CalculateColorVariance(float alpha, vec2 lightPixelCoord, vec2 currentPixelCoord, float distance) {
    float variance = 0;
        ...
        if (lightPixelCoord.x > currentPixelCoord.x) {
            for (float i = currentPixelCoord.x + 1; i <= lightPixelCoord.x; i += 1) {
                ...
                if (currentColor.a > alpha) {
                    variance += Shadow(Circle(vec2(i, i * a + b), currentPixelCoord), currentColor.a - alpha, distance);
                } else if (leftColor.a > alpha && verColor.a > alpha) {
                    variance += Shadow(Circle(vec2(i, i * a + b), currentPixelCoord), leftColor.a - alpha, distance);
                }
            }
        } else {
            for (float i = currentPixelCoord.x - 1; i >= lightPixelCoord.x; i -= 1) {
                ...
                if (currentColor.a > alpha) {
                    ...
                    variance += Shadow(Circle(vec2(i, i * a + b), currentPixelCoord), rightColor.a - alpha, distance);
                }
            }
        }
    } else {
        ...
        if (a != 0) {
            if (lightPixelCoord.y > currentPixelCoord.y) {
                for (float i = currentPixelCoord.y + 1; i <= lightPixelCoord.y; i += 1) {
                    ...
                    if (currentColor.a > alpha) {
                        variance += Shadow(Circle(vec2((i - b) / a, i), currentPixelCoord), currentColor.a - alpha, distance);
                    } else if ((downColor.a > alpha && horColor.a > alpha)) {
                        variance += Shadow(Circle(vec2((i - b) / a, i), currentPixelCoord), downColor.a - alpha, distance);
                    }
                }
            } else {
                for (float i = currentPixelCoord.y - 1; i >= lightPixelCoord.y; i -= 1) {
                    ...
                    if (currentColor.a > alpha) {
                        ...
                        variance += Shadow(Circle(vec2((i - b) / a, i), currentPixelCoord), upColor.a - alpha, distance);
                    }
                }
            }
        } else {
            if (lightPixelCoord.y > currentPixelCoord.y) {
                for (float i = currentPixelCoord.y + 1; i <= lightPixelCoord.y; i += 1) {
                    ...
                    if (currentColor.a > alpha) {
                        variance += Shadow(Circle(vec2(lightPixelCoord.x, i), currentPixelCoord), currentColor.a - alpha, distance);
                    }
                }
            } else {
                for (float i = currentPixelCoord.y - 1; i >= lightPixelCoord.y; i -= 1) {
...
void main() {
    ...
    float variance = CalculateColorVariance(color.a, lightCoord, pos, circle);

    if (circle > 0 && variance < 10 && variance >= 0) {
        variance = (circle - variance * 5) / 500;
